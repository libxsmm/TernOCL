...
// TernOCL copy: rotating sets sized like the OpenCL driver -- distinct weights
// (B uint32 words + fp32 SB of this harness) >= g_weights_gib GiB, or g_sets.
double g_weights_gib = 2.0;
int g_sets = 0;
int calculate_n_sets_runtime() {
    if (g_sets > 0) return g_sets;
    const double w = (double)g_mat_k * g_mat_n / 16 * 4 + (double)(g_mat_k / 128) * g_mat_n * 4;
...
int gemm_result_validate(data_type_a *A, data_type_b *B, data_type_c *C, uint32_t m, uint32_t k, uint32_t n, sycl::context &context, uint32_t scale_gs, const float *scaleA, size_t size_scale_a, const float *scaleB, size_t size_scale_b,
    ...
    // Compute gold reference in float and then convert to data_type_c (may be bf16)
...
#pragma omp parallel for collapse(2)
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < n; j++) {
            ...
            for (int gk = 0; gk < ks_groups; gk++) {
                int32_t acc_int = 0;
                float cur_scale_a = frcp(scaleA[gk * m + i]);
                float cur_scale_b = scaleB[gk * n + j];
...
void int2_dequantize_gemm_run(int iter, int n_sets, bool enable_validation) {
    ...
    // Scale buffers (float) per set
    std::vector<float *> ScaleA_h(n_sets), ScaleA_d(n_sets);
    std::vector<float *> ScaleB_h(n_sets), ScaleB_d(n_sets);
    ...
    for (int set = 0; set < n_sets; ++set) {
        ...
        ScaleA_h[set] = static_cast<float *>(malloc_host(size_scale_a * sizeof(float), context));
        ScaleB_h[set] = static_cast<float *>(malloc_host(size_scale_b * sizeof(float), context));
        ...
        ScaleA_d[set] = static_cast<float *>(aligned_alloc_device(DEVICE_MEM_ALIGNMENT, size_scale_a * sizeof(float), device, context));
        ScaleB_d[set] = static_cast<float *>(aligned_alloc_device(DEVICE_MEM_ALIGNMENT, size_scale_b * sizeof(float), device, context));
...
#pragma omp parallel for
        for (unsigned i = 0; i < size_a; ++i) {
            ...
            A_h[set][i] = static_cast<bf16>(generate_real_random<float>(-5.0f, 5.0f));
        }
#pragma omp parallel for
        ...
        for (size_t si = 0; si < size_scale_b; ++si) {
            ScaleB_h[set][si] = generate_real_random<float>(0.0f, 15.0f) + 0.75f; // example B-scale
        }
...
#pragma omp parallel for
    ...
    for (int set = 0; set < n_sets; ++set) {
        ...
        //queue.memcpy((void *)ScaleA_d[set], (void *)ScaleA_h[set], size_scale_a * sizeof(float)).wait();
        queue.memcpy((void *)ScaleB_d[set], (void *)ScaleB_h[set], size_scale_b * sizeof(float)).wait();
        ...
        for (int i = 0; i < iter; i++) {
            ...
            for (int set = 0; set < n_sets; ++set) {
                ...
                if constexpr (use_external_scale_a) {
                    ...
                    const size_t num_groups = (rows + TILE_Y - 1) / TILE_Y; // number of groups needed
...
#if 0
                    queue.memcpy((void *)ScaleA_d[set], (void *)ScaleA_h[set], size_scale_a * sizeof(float)).wait();
                            //std::cout << "[debug] compute_scale kernel finished for set " << set << "" << std::endl;
                            prof.add_gpu_event(e_scale);
#endif
                    if (i >= 1) {
...
#if 0
                    // Compare here element by element the host and device scales to make sure they are identical by using a temp host side buffer to copy over the device
                    float *ScaleA_temp_h = static_cast<float *>(malloc_host(size_scale_a * sizeof(float), context));
                    std::cout << "[debug] copying device scales to host for set " << set << " (size=" << size_scale_a << ")" << std::endl;
                    queue.memcpy((void *)ScaleA_temp_h, (void *)ScaleA_d[set], size_scale_a * sizeof(float)).wait();
                    std::cout << "[debug] copy complete for set " << set << "" << std::endl;

                    int correct = 1;
                    for (size_t si = 0; si < size_scale_a; ++si) {
                        if (ScaleA_temp_h[si] != ScaleA_h[set][si]) {
                            std::cout << "ScaleA mismatch at index " << si << ": host="
                                      << std::fixed << std::setprecision(9) << ScaleA_h[set][si]
                                      << ", device=" << std::fixed << std::setprecision(9) << ScaleA_temp_h[si]
                                      << std::endl;
                            correct = 0;
                        }
                    }
                    if (set == 0 && 0) {
                        // prind side by side all scales from host and device
                        std::cout << "ScaleA values (host vs device):\n";
                        for (size_t si = 0; si < size_scale_a; ++si) {
                            std::cout << "Index " << si << ": " << ScaleA_h[set][si] << " vs " << ScaleA_temp_h[si] << "\n";
                        }
                    }
                    if (!correct) {
                        std::cout << "ScaleA validation FAILED for set " << set << std::endl;
                        FAIL();
                    } else {
                        std::cout << "ScaleA validation PASSED for set " << set << std::endl;
                    }
                    free(ScaleA_temp_h, context);
#endif
                }
    ...
    double device_bytes_per_set = (double)((matrix_k / 4) * matrix_n + matrix_k * matrix_m * sizeof(data_type_a) + (size_scale_a + size_scale_b) * sizeof(float));
...
static void print_precompile_plan() {
    ...
    for (size_t im = 0; im < PRE_GEMM_WG_M.size(); ++im) {
        ...
        for (size_t in = 0; in < PRE_GEMM_WG_N_PAIRS.size(); ++in) {
            ...
            std::cout << "  int2_dequant_base<" << PRECOMP_GKS << ", " << wg_m << ", " << sg_m << ", " << wg_n << ", " << sg_n << ", " << sg_k << ", " << sg_m << ", int32_t, bf16>\n";
        }
    }
    ...
    for (size_t in = 0; in < PRE_GEMV_WG_N_PAIRS.size(); ++in) {
        ...
        for (size_t ik = 0; ik < PRE_GEMV_SG_K.size(); ++ik) {
            ...
            std::cout << "  int2_dequant_base<" << PRECOMP_GKS << ", " << PRE_GEMV_WG_M << ", " << PRE_GEMV_SG_M << ", " << wg_n << ", " << sg_n << ", " << sg_k << ", 1, int32_t, bf16>\n";
        }
    }
}
#endif

// Explicit template instantiations for all precompiled variants
// These provide the actual function implementations referenced by the variant registry

// Macro to instantiate all 4 post-op combinations for a given Test type base with both scale modes
// This creates 8 instantiations: 4 post-op combos × 2 scale modes (external/internal)
// Usage: INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 256u, 128u, 32u, 8u, int32_t, bf16)
#if 1
#define INSTANTIATE_ALL_POSTOPS(GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType) \
    ...
    template void int2_dequantize_gemm_run<int2_dequant_base<GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType, false>, false, false>(int, int, bool); \
    template void int2_dequantize_gemm_run<int2_dequant_base<GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType, false>, true, false>(int, int, bool); \
    template void int2_dequantize_gemm_run<int2_dequant_base<GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType, false>, false, true>(int, int, bool); \
    template void int2_dequantize_gemm_run<int2_dequant_base<GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType, false>, true, true>(int, int, bool);
#else
#define INSTANTIATE_ALL_POSTOPS(GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType) \
    template void int2_dequantize_gemm_run<int2_dequant_base<GKS, WgM, SgM, WgN, SgN, SgK, XMXM, AccType, CType, true>, false, false>(int, int, bool);
#endif
...
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 128u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 160u, 160u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 256u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 128u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 160u, 160u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 256u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 128u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 160u, 160u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 256u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 128u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 160u, 160u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 256u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 128u, 128u, 32u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 160u, 160u, 32u, 8u, int32_t, bf16)

#if 1
//sg_k = 64
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 256u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 128u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 160u, 160u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 256u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 128u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 160u, 160u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 256u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 128u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 160u, 160u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 256u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 128u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 160u, 160u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 256u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 128u, 128u, 64u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 160u, 160u, 64u, 8u, int32_t, bf16)

//sg_k = 96
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 256u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 128u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 160u, 160u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 256u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 128u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 160u, 160u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 256u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 128u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 160u, 160u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 256u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 128u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 160u, 160u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 256u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 128u, 128u, 96u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 160u, 160u, 96u, 8u, int32_t, bf16)

//sg_k = 128
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 256u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 128u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 160u, 160u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 256u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 128u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 160u, 160u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 256u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 128u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 160u, 160u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 256u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 128u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 160u, 160u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 256u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 128u, 128u, 128u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 160u, 160u, 128u, 8u, int32_t, bf16)

//sg_k = 160
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 256u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 128u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 16u, 8u, 160u, 160u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 256u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 128u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 32u, 8u, 160u, 160u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 256u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 128u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 64u, 8u, 160u, 160u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 256u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 128u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 128u, 8u, 160u, 160u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 256u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 128u, 128u, 160u, 8u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 256u, 8u, 160u, 160u, 160u, 8u, int32_t, bf16)
#endif

// GEMV instantiations (48 total: 10 (wg_n, sg_n) pairs × 4 sg_k values + 2 remaining)
// (16, 16) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 16u, 16u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 16u, 16u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 16u, 16u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 16u, 16u, 256u, 1u, int32_t, bf16)

// (32, 32) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 32u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 32u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 32u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 32u, 256u, 1u, int32_t, bf16)

// (32, 16) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 16u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 16u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 16u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 32u, 16u, 256u, 1u, int32_t, bf16)

// (64, 64) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 64u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 64u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 64u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 64u, 256u, 1u, int32_t, bf16)

// (64, 32) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 32u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 32u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 32u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 32u, 256u, 1u, int32_t, bf16)

// (64, 16) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 16u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 16u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 16u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 64u, 16u, 256u, 1u, int32_t, bf16)

// (128, 128) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 128u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 128u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 128u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 128u, 256u, 1u, int32_t, bf16)

// (128, 64) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 64u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 64u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 64u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 64u, 256u, 1u, int32_t, bf16)

// (128, 32) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 32u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 32u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 32u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 32u, 256u, 1u, int32_t, bf16)

// (128, 16) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 16u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 16u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 16u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 128u, 16u, 256u, 1u, int32_t, bf16)

// (80, 80) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 80u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 80u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 80u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 80u, 256u, 1u, int32_t, bf16)

// (80, 32) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 32u, 32u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 32u, 128u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 32u, 160u, 1u, int32_t, bf16)
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 32u, 256u, 1u, int32_t, bf16)

// (80, 16) with sg_k variants
INSTANTIATE_ALL_POSTOPS(1u, 1u, 1u, 80u, 16u, 32u, 1u, int32_t, bf16)
